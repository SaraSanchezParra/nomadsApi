        ...
        if (err) {
          ...
          connection.query(sqlU, (err, res) => {
            if (err) {
              ...
              excursion.user_foto = res[0].photo;
              console.log(res);
              answer = {
                error: false,
                codigo: 200,
...
function getPIOfDay(req, response) {
  ...
  let sql =
    "SELECT d.viaje_id, p.nombre, p.foto, p.corLong, p.corLat FROM nomads.dias as d join puntos_de_interes as p on (d.dia_id = p.dia_id) where d.dia_id = ?;";
  connection.query(sql, params, (err, res) => {
    if (err) {
      answer = { error: true, codigo: 200, mensaje: err, data_viaje: [null] };
    } else {
...
function getTopViajesLog(request, response) {
  let respuesta;
  let sql =
    "SELECT viajes.titulo, viajes.descripcion, viajes.foto, COUNT(*) AS likes FROM favoritos JOIN viajes ON viajes.viaje_id = favoritos.viaje_id_fav GROUP BY viajes.viaje_id ORDER BY likes DESC LIMIT 4";
...
function postViaje(req, response) {
  let sql =
    "INSERT INTO nomads.viajes (titulo, descripcion, ubicacion, foto, user_id_propietario, n_dias_viaje, n_likes)" +
    "VALUES ('" +
    req.body.titulo +
    "', '" +
    ...
    if (err) {
      ...
      res
        ...
        .json({
          error: true,
          codigo: 500,
          mensaje: "Error al insertar día en la base de datos",
        });
    } else {
      res
        .status(201)
        .json({
          error: false,
          codigo: 201,
          mensaje: "Día insertado correctamente",
...
function viajes(request, response) {
  ...
  let sql =
    "SELECT foto, titulo, viajes.descripcion, n_likes, user.photo FROM viajes " +
...
function getTopNomads(request, response) {
  let respuesta;
  let sql =
    "SELECT user.photo, user.username, COUNT(*) AS likes FROM favoritos JOIN viajes ON viajes.viaje_id = favoritos.viaje_id_fav JOIN user ON user.user_id = viajes.user_id_propietario GROUP BY user.user_id, user.photo ORDER BY likes DESC LIMIT 4";
...
function addLike(req, response) {
  let sql =
    "INSERT INTO nomads.favoritos (user_id_fav, viaje_id_fav) VALUES ('" +
    req.body.user_id +
    "', '" +
    req.body.viaje_id +
    "'); ";
...
function removeLike(req, res) {
  ...
  let sql =
    "DELETE FROM nomads.favoritos WHERE (viaje_id_fav = ?) AND (user_id_fav = ?);";
  connection.query(sql, params, (err, res) => {
    if (err) {
      console.log(err);
      respuesta = {
