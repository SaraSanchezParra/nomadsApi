...
function postRegister(request, response) {
  ...
  let sqlCheckUser = "SELECT COUNT(*) as count FROM user WHERE username='" + username + "'";
  connection.query(sqlCheckUser, function (err, result) {
    if (err) {
      console.log(err);
      respuesta = { error: true, codigo: 200, mensaje: 'No registrado', data_user: result }
      response.json(respuesta)
    }
    else if (result[0].count > 0) {
      console.log("Nombre de usuario ya existe");
      respuesta = { error: true, codigo: 200, mensaje: 'Nombre de usuario ya existe', data_user: result }
      response.json(respuesta)
    }
    else {
